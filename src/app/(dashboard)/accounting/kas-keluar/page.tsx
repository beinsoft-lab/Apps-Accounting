...
import { Plus, Send, Ban, Eye, ArrowUpFromLine } from 'lucide-react'
...
async function fetchCash(filterStatus: string) {
  const p = new URLSearchParams({ type: 'OUT', limit: '100' })
...
export default function KasKeluarPage() {
  ...
  const { data: resp, isLoading, isError, refetch } = useQuery({
    queryKey: ['cash', 'OUT', filterStatus],
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h2 className="text-headline-lg font-bold text-on-surface">Kas Keluar</h2>
          <p className="text-body-md text-body-grey mt-1">Pencatatan pengeluaran kas dan bank</p>
        </div>
        <button
        ...
        >
          <Plus className="w-4 h-4" /> Tambah Kas Keluar
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="card p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-label-sm text-body-grey">Total Dikeluarkan (Posted)</p>
            <p className="text-xl font-bold text-error">Rp {fmtIDR(meta?.totalAmountOut ?? 0)}</p>
          </div>
        </div>
          ...
          <DataTable<CashTransaction>
            ...
            columns={[
              ...
              },
              {
                ...
                align:  'right',
                accessor: t => (
                  <span className="font-semibold text-error">Rp {fmtIDR(t.amount)}</span>
                ),
              },
              {
                header: 'Status',
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
      <div className="bg-surface-bright rounded-xl shadow-overlay w-full max-w-2xl my-8 p-8 space-y-6 animate-fade-in">
        <h2 className="text-headline-md font-bold text-on-surface">Tambah Kas Keluar</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
            <label className="text-label-sm text-on-surface-variant">Akun Kas (Sumber) *</label>
            <select
          ...
          <div className="space-y-1">
            <label className="text-label-sm text-on-surface-variant">Akun Tujuan *</label>
            <select
              value={form.counterpartAccountId}
              onChange={e => setForm(p => ({ ...p, counterpartAccountId: e.target.value }))}
              className="input-field"
            >
              <option value="">— Ke akun mana? —</option>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder="Pembayaran sewa kantor"
            className="input-field"
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
            <label className="text-label-sm text-on-surface-variant">Dibayar Kepada</label>
            <input
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="PO-001, dsb. (opsional)"
              className="input-field"
            />
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="py-1">{selectedCounterpart ? `${selectedCounterpart.code} - ${selectedCounterpart.name}` : '—'}</td>
                  <td className="py-1 text-right font-semibold text-error">Rp {fmtIDR(amountNum)}</td>
                  <td className="py-1 text-right text-body-grey">—</td>
                </tr>
                <tr>
                  <td className="py-1 text-on-surface-variant font-semibold text-xs">Cr</td>
                  <td className="py-1 pl-4">{selectedCash ? `${selectedCash.code} - ${selectedCash.name}` : '—'}</td>
                  <td className="py-1 text-right text-body-grey">—</td>
                  <td className="py-1 text-right font-semibold text-error">Rp {fmtIDR(amountNum)}</td>
                </tr>
              </tbody>
      ...
      <div className="bg-surface-bright rounded-xl shadow-overlay w-full max-w-2xl p-8 space-y-6 animate-fade-in max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3 text-label-sm">
          ...
          <div><span className="text-body-grey">Nominal:</span>{' '}
            <span className="font-bold text-error">Rp {fmtIDR(tx.amount)}</span>
          </div>
          ...
          <div><span className="text-body-grey">Akun Tujuan:</span>{' '}
            <span className="font-data-mono">{tx.counterpartAccount.code} - {tx.counterpartAccount.name}</span>
          </div>
          {tx.partyName && (
            <div><span className="text-body-grey">Dibayar kepada:</span>{' '}
